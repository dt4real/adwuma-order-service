...
serviceName = "adwuma-order"
service = "Adwuma Order"
...
pipeline {
  ...
  environment {
    ...
    IMAGE_NAME = "dt4real" + "/" + "adwuma-order"
  ...
  stages {
    ...
    stage("Prepare Environment") {
      steps {
        ...
        dir("pipeline") {
          ...
          git branch: 'main', credentialsId: 'github', url: 'https://github.com/dt4real/adwuma-jenkins-automation'
          script {
            groovyMethods = load("functions.groovy")
          }
        }
        // Add your order github url to url field
        git branch: 'main', credentialsId: 'github', url: 'https://github.com/dt4real/adwuma-order-service'
        sh 'npm install'
      }
    }
    ...
    stage("Build and Push") {
      steps {
        sh 'docker login -u $DOCKERHUB_CREDENTIAL_USR --password $DOCKERHUB_CREDENTIALS_TOKEN'
    ...
    stage("Create New Pods") {
      steps {
        withKubeCredentials(kubectlCredentials: [[caCertificate: '', clusterName: 'minikube', contextName: 'minikube', credentialsId: 'jenkins-k8s-token', namespace: 'production', serverUrl: 'https://172.21.176.116:8443']]) {
          script {
